   ...
   /* Verification de la connexion */
	...
	$jour = date('Y-m-d', time());  
        ...
        if ($jour > $last_conso){
			...
			$result = mysqli_query($link,$SQLrequete);
...
$data[0] = date('Y-m-d H:i:s', time()); //remplace le premier parametre (pm) par la date // Le fuseau horaire se règle dans config.inc.php
//echo $data[0]; //DEBUG
// var_dump($data);//DEBUG
// echo sizeof($data);//DEBUG
...
$result = mysqli_query($link,$requete); // ecriture en BDD

if (!$result) { // si requete KO on log et on quitte
	addLogEvent($requete);	
    die('erreur ecriture : ' . mysqli_error($link));
}
